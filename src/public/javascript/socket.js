...
function add_bounding_box(topic, image, rect, id) {
    socket.emit('add bounding_box', {topic: topic, image: image, bounding_box: rect, id: id}, (res) => {
        if (res.indexOf('error') >= 0) {
            console.error(res);
            return;
        }
        ...
        if (local_bounding.checked) {
            old_bounding_box.forEach(rect => {
                if (rect.id >= 0 && !exist_bounding_box_by_id(res, rect.id))
                    add_bounding_box(select_topic.value, image_numbers[index], rect.rect, rect.id);
            });
        }
        get_bounding_box(select_topic.value, image_numbers[index]);
    });
